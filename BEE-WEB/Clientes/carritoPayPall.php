...
<head>
    ...
    <script src="https://sandbox.paypal.com/sdk/js?client-id=Ad-08zvevPL9L3ofplbSp_kRW-7H-YUtMfJqZrDtlV0npBj920BHT76drLOdDVyBGs2QTodkzB9Fu5s9&currency=MXN"></script>
</head>
<body>
    <!-- Start Top Nav -->
    ...
    <!-- Close Top Nav -->
     <?php if($prodCarrito > 0){ ?>  
        <?php echo "<script>
        // Obtener el valor del parámetro 'pago' en la URL
        const urlParams = new URLSearchParams(window.location.search);
        const pago = urlParams.get('pago');

        // Si el pago fue exitoso, mostrar el mensaje
        if (pago === 'exitoso') {
            alert('Producto comprado correctamente, te llegara a tu ubicación en 3 dias');
        }
        </script>";?> 
                    ...
                    <div class="col-lg-7 mt-5">
                        <div class="card d-flex justify-content-center">
                            <div class="card-body position-relative"> 
                                ...
                                <!-- <div id="paypal-button-container" class="pb-5"></div> -->
                ...
                $(document).ready(function() {
                    ...
                    paypal.Buttons({
                        ...
                        createOrder: function(data, actions) {
                            // Obtener el monto dinámico desde el campo correspondiente
                            let montoDinamico = parseFloat($("#product-actu<?php echo $id; ?>").val()); // Campo donde se calcula el total dinámico
                            console.log("Monto dinámico calculado: ", montoDinamico); // Para depurar
                            return actions.order.create({
                                purchase_units: [{ // Corrige "purchase_unit" a "purchase_units"
                                    amount: { // Corrige "amoutn" a "amount"
                                        value: montoDinamico.toFixed(2) // Convertir a string y limitar a 2 decimales
                                    }
                                }]
                            });
                        },
                        onApprove: function(data,actions){
                            return actions.order.capture().then(function (detalles){
                                ...
                                // Realizar la solicitud POST después de la aprobación del pago
                                const carritoId = <?php echo $id; ?>;
                                const cantidad = parseInt($("#var-value<?php echo "$id"?>").html());
                                const nombreProducto = "<?php echo "$strProd"; ?>"; 
                                const saldo = parseFloat($("#product-actu<?php echo "$id"?>").val());
                                const imagen = "<?php echo "$imgCarr"?>";
                                const tope = parseInt($("#top<?php echo $id ?>").val());
                                const dif = tope - cantidad;
                                console.log(dif);
                                console.log(saldo);

                                const miCantidad = parseFloat(<?php echo $miDinero ?>);
                                console.log(miCantidad);
                                const compara = miCantidad - saldo;
                                console.log(compara);
                                ...
                                if (compara < 0) {
                                    ...
                                    $.post('../assets/scripts/compraProducto.php', {
                                            ...
                                            if (data.success) {   
                                                $('#cont<?php echo "$id"?>').remove(); // Eliminar el producto del carrito
                                                window.location.href = "carrito.php?pago=exitoso";
                                            } else {
                    ...
                    $('#elimina<?php echo "$id"?>').click(function(event){
                        ...
                        if (decision) {
                            $.post('../assets/scripts/eliminaCarrito.php', { productoId: productoId }, function() {
                                ...
                                window.location.href = "carrito.php?pago=borrado";
                            })
                            .fail(function() {  
